...
class MY_JSON
{
    ...
    public function encode($var)
    {
        ...
        case 'object':
            ...
            return ('{' . join(',', $properties)) . '}';
        default:
            return $this->use & MY_JSON_SUPPRESS_ERRORS ? 'null' : new MY_JSON_Error(gettype($var) . ' can not be encoded as JSON string');
        }
    }
    public function name_value($name, $value)
    {
        $encoded_value = $this->encode($value);
        if (MY_JSON::isError($encoded_value)) {
    ...
    {
        if (is_object($data) && (get_class($data) == 'MY_JSON_error' || is_subclass_of($data, 'MY_JSON_error'))) {
            return true;
        }
        return false;
    }
...
class MY_JSON_Error
{
    public function MY_JSON_Error($message = 'unknown error', $code = null, $mode = null, $options = null, $userinfo = null)
    {
